...
class YOLOv11Detector:
    ...
    def __init__(self, working_dir: str, confidence_threshold: float = 0.8):
        ...
        self.class_names = ['CASE_B', 'CASE_F', 'STACK']  # 3種類型
    ...
    def load_calibration_data(self, camera_matrix_file: str = None, 
        ...
        try:
            ...
            if camera_matrix_file:
                ...
                # 修正: 根據測試結果，camera_matrix_CASE.npy是直接的3x3數組
            ...
            if dist_coeffs_file and dist_coeffs is None:
                ...
                # 修正: 根據測試結果，dist_coeffs_CASE.npy是(1,5)數組
                if hasattr(dist_data, 'shape'):
            ...
            if extrinsic_file:
                ...
                # 修正: 根據測試結果，extrinsic_CASE.npy是0維數組包含字典
...
class EnhancedModbusTcpClientService:
    ...
    def __init__(self, server_ip="127.0.0.1", server_port=502):
        ...
        self.REGISTERS = {
            ...
            # 擴展檢測結果 (255-259)
            'CASE_B_1_X': 255,            # CASE_B第1個座標X
            'CASE_B_1_Y': 256,            # CASE_B第1個座標Y
            ...
            'CASE_F_1_WORLD_X_HIGH': 261,
            'CASE_F_1_WORLD_X_LOW': 262,
            'CASE_F_1_WORLD_Y_HIGH': 263,
            'CASE_F_1_WORLD_Y_LOW': 264,
            'CASE_F_2_WORLD_X_HIGH': 265,
            'CASE_F_2_WORLD_X_LOW': 266,
            'CASE_F_2_WORLD_Y_HIGH': 267,
            'CASE_F_2_WORLD_Y_LOW': 268,
            'CASE_F_3_WORLD_X_HIGH': 269,
            'CASE_F_3_WORLD_X_LOW': 270,
            'CASE_F_3_WORLD_Y_HIGH': 271,
            'CASE_F_3_WORLD_Y_LOW': 272,
            'CASE_F_4_WORLD_X_HIGH': 273,
            'CASE_F_4_WORLD_X_LOW': 274,
            'CASE_F_4_WORLD_Y_HIGH': 275,
            'CASE_F_4_WORLD_Y_LOW': 276,
            'CASE_F_5_WORLD_X_HIGH': 277,
            'CASE_F_5_WORLD_X_LOW': 278,
            'CASE_F_5_WORLD_Y_HIGH': 279,
            'CASE_F_5_WORLD_Y_LOW': 280,
        ...
        try:
            ...
            for i in range(5):
                if i < len(result.case_f_coords):
                    ...
                    print(f"   {245+i*2}(CASE_F_{i+1}_X) = {int(float(x))}")
                    print(f"   {246+i*2}(CASE_F_{i+1}_Y) = {int(float(y))}")
                else:
                    self.write_register(f'CASE_F_{i+1}_X', 0)
                    self.write_register(f'CASE_F_{i+1}_Y', 0)
            
            # 寫入第一個CASE_B和STACK座標 (新增)
            if result.case_b_coords:
                ...
                print(f"   255(CASE_B_1_X) = {int(float(x))}")
                print(f"   256(CASE_B_1_Y) = {int(float(y))}")
            else:
            ...
            if (result.success and result.case_f_coords and
                ...
                if world_coords:
                    ...
                    for i in range(min(5, len(world_coords))):
                        ...
                        print(f"   處理 CASE_F {i+1}: 原始值=({world_x:.2f}, {world_y:.2f})")
                    ...
                    print(f"✅ 共寫入{len(world_coords)}個CASE_F世界座標到Modbus寄存器")
...
class CCD1VisionController:
    ...
    def capture_image(self) -> Tuple[Optional[np.ndarray], float]:
        ...
        try:
            # 步驟1：執行軟體觸發
            self.logger.info("執行軟體觸發...")
            trigger_result = self.camera_manager.trigger_software(["ccd1_camera"])
            self.logger.info(f"觸發結果: {trigger_result}")
            
            if not trigger_result.get("ccd1_camera", False):
                self.logger.error("軟體觸發失敗")
                return None, 0.0
            
            # 步驟2：等待並捕獲觸發後的幀
            self.logger.info("等待並捕獲觸發後的幀...")
            # 軟體觸發後需要稍微等待讓相機準備
            time.sleep(0.1)
            ...
            if frame_data is None:
                self.logger.error("軟體觸發後無法捕獲幀")
                return None, 0.0
            
            capture_time = time.time() - capture_start
            self.logger.info(f"軟體觸發拍照成功，耗時: {capture_time*1000:.2f}ms")
            
            image_array = frame_data.data
            self.logger.info(f"圖像數據: 形狀={image_array.shape}, 類型={image_array.dtype}")
            
            # 根據像素格式處理圖像
            if len(image_array.shape) == 2:
                # 灰度圖像轉BGR
                display_image = cv2.cvtColor(image_array, cv2.COLOR_GRAY2BGR)
                self.logger.info("轉換灰度圖像為BGR格式")
            else:
                display_image = image_array
                self.logger.info("使用原始BGR圖像")
            
            return display_image, capture_time
            
        except Exception as e:
            capture_time = time.time() - capture_start
            self.logger.error(f"軟體觸發拍照異常: {e}")
            import traceback
            self.logger.error(f"詳細錯誤堆疊: {traceback.format_exc()}")
    ...
    def _create_yolo_visualization(self, image: np.ndarray, result: YOLODetectionResult):
        ...
        try:
            ...
            for i, (x, y) in enumerate(result.case_b_coords):
                ...
                label = f"CASE_B {i+1}"
                cv2.putText(vis_image, label, (int(x-30), int(y-25)), 
            ...
            # 添加檢測統計信息
            stats_text = f"YOLOv11 (Model{result.model_id_used}): F={result.case_f_count}, B={result.case_b_count}, S={result.stack_count}"
...
def capture_and_detect():
    ...
    try:
        ...
        # 🔥 調試：打印響應數據
        print(f"📤 API響應數據檢查:")
        print(f"   CASE_F: {response['case_f_count']}")
        print(f"   CASE_B: {response['case_b_count']}")
...
def get_modbus_registers():
    ...
    try:
        ...
        # 🔥 新增：CASE_F座標寄存器 (245-254)
        for i in range(1, 6):
            case_f_x = modbus_client.read_register(f'CASE_F_{i}_X')
            case_f_y = modbus_client.read_register(f'CASE_F_{i}_Y')
            if case_f_x is not None and case_f_y is not None and (case_f_x != 0 or case_f_y != 0):
                registers[f'{245+(i-1)*2}_CASE_F_{i}_X'] = case_f_x
                registers[f'{246+(i-1)*2}_CASE_F_{i}_Y'] = case_f_y
        
        # 🔥 新增：擴展檢測結果 (255-259)
        case_b_x = modbus_client.read_register('CASE_B_1_X')
        case_b_y = modbus_client.read_register('CASE_B_1_Y')
        if case_b_x is not None and case_b_y is not None and (case_b_x != 0 or case_b_y != 0):
            registers['255_CASE_B_1_X'] = case_b_x
            registers['256_CASE_B_1_Y'] = case_b_y
        ...
        if world_coord_valid:
            for i in range(1, 6):
                ...
                if x_high != 0 or x_low != 0 or y_high != 0 or y_low != 0:
                    ...
                    registers[f'{261+(i-1)*4}_CASE_F_{i}_世界X'] = f"{world_x_mm:.2f}mm"
                    registers[f'{263+(i-1)*4}_CASE_F_{i}_世界Y'] = f"{world_y_mm:.2f}mm"
