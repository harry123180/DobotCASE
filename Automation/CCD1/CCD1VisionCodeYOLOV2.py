# -*- coding: utf-8 -*-
"""
CCD1VisionCode_CASE_Enhanced_NoWeb.py - CCD1視覺控制系統 CASE增強版
整合YOLOv11物件檢測功能，支援CASE_B/CASE_F/STACK三種分類檢測
移除Web介面，純Modbus架構，增加記憶體監控、系統重載、圖片保存控制
適配pymodbus 3.9.2
"""
...
class YOLODetectionResult:
    """YOLOv11檢測結果 - CASE_B/CASE_F/STACK分類"""
    success: bool = False
    case_f_count: int = 0
    case_b_count: int = 0
    stack_count: int = 0
    ...
    case_f_confidences: List[float] = None
    case_b_confidences: List[float] = None
    ...
    def __post_init__(self):
        ...
        if self.stack_coords is None:
            self.stack_coords = []
        if self.case_f_confidences is None:
            self.case_f_confidences = []
        if self.case_b_confidences is None:
            self.case_b_confidences = []
...
class EnhancedModbusTcpClientService:
    """增強版Modbus TCP Client服務 - CASE增強版 (移除Web介面)"""
    ...
    def __init__(self, server_ip="127.0.0.1", server_port=502):
        ...
        # CCD1 Modbus寄存器映射 (基地址200) - CASE增強版
        self.REGISTERS = {
            # 控制寄存器 (200-201)
            'CONTROL_COMMAND': 200,
            'STATUS_REGISTER': 201,
            ...
            # CASE_F座標寄存器 (245-254)
            'CASE_F_1_X': 245, 'CASE_F_1_Y': 246,
            'CASE_F_2_X': 247, 'CASE_F_2_Y': 248,
            'CASE_F_3_X': 249, 'CASE_F_3_Y': 250,
            'CASE_F_4_X': 251, 'CASE_F_4_Y': 252,
            'CASE_F_5_X': 253, 'CASE_F_5_Y': 254,
            
            # CASE_B和STACK座標寄存器 (255-259)
            'CASE_B_1_X': 255,
            'CASE_B_1_Y': 256,
            'STACK_1_X': 257,
            'STACK_1_Y': 258,
            ...
            'CASE_F_1_WORLD_X_HIGH': 261, 'CASE_F_1_WORLD_X_LOW': 262,
            'CASE_F_1_WORLD_Y_HIGH': 263, 'CASE_F_1_WORLD_Y_LOW': 264,
            'CASE_F_2_WORLD_X_HIGH': 265, 'CASE_F_2_WORLD_X_LOW': 266,
            'CASE_F_2_WORLD_Y_HIGH': 267, 'CASE_F_2_WORLD_Y_LOW': 268,
            'CASE_F_3_WORLD_X_HIGH': 269, 'CASE_F_3_WORLD_X_LOW': 270,
            'CASE_F_3_WORLD_Y_HIGH': 271, 'CASE_F_3_WORLD_Y_LOW': 272,
            'CASE_F_4_WORLD_X_HIGH': 273, 'CASE_F_4_WORLD_X_LOW': 274,
            'CASE_F_4_WORLD_Y_HIGH': 275, 'CASE_F_4_WORLD_Y_LOW': 276,
            'CASE_F_5_WORLD_X_HIGH': 277, 'CASE_F_5_WORLD_X_LOW': 278,
            'CASE_F_5_WORLD_Y_HIGH': 279, 'CASE_F_5_WORLD_Y_LOW': 280,
            ...
            'SYSTEM_RELOAD': 296,
            'RELOAD_STATUS': 297,
            'SAVE_IMAGE_CONTROL': 298,    # 圖片保存控制
            'SAVE_IMAGE_MODE': 299,       # 保存模式設定
        }
    ...
    def _write_initial_status_and_defaults(self):
        ...
        try:
            ...
            # 版本資訊 (CASE增強版)
            self.write_register('VERSION_MAJOR', 5)  # CASE增強版本
            self.write_register('VERSION_MINOR', 2)  # 增強版本號
    ...
    def _check_and_write_defaults(self):
        ...
        try:
            ...
            # 檢查圖片保存模式
            save_mode = self.read_register('SAVE_IMAGE_MODE')
            if save_mode is None or save_mode == 0:
                print("📝 圖片保存模式為0，寫入預設值: 1 (SUCCESS_ONLY)")
                self.write_register('SAVE_IMAGE_MODE', ImageSaveMode.SUCCESS_ONLY)
            else:
                print(f"📋 圖片保存模式現有值: {save_mode}")
    ...
    def _handle_capture_detect_command_enhanced(self):
        ...
        try:
            ...
            if result and result.success:
                ...
                print(f"✅ YOLOv11檢測成功，CASE_F={result.case_f_count}, CASE_B={result.case_b_count}, STACK={result.stack_count}")
                return True
            else:
    ...
    def _clear_detection_results(self):
        ...
        try:
            ...
            # 清空CASE_B和STACK座標
            self.write_register('CASE_B_1_X', 0)
            self.write_register('CASE_B_1_Y', 0)
    ...
    def update_detection_results(self, result: YOLODetectionResult):
        ...
        try:
            ...
            # 寫入CASE_B座標 (第1個)
            if result.case_b_coords:
                x, y = result.case_b_coords[0]
                self.write_register('CASE_B_1_X', int(float(x)))
                self.write_register('CASE_B_1_Y', int(float(y)))
            else:
                self.write_register('CASE_B_1_X', 0)
                self.write_register('CASE_B_1_Y', 0)
...
class CCD1VisionController:
    ...
    def __init__(self):
        ...
        self.image_save_mode = ImageSaveMode.SUCCESS_ONLY  # 預設SUCCESS_ONLY模式
    ...
    def _save_detection_image(self, vis_image: np.ndarray, result: YOLODetectionResult):
        ...
        try:
            ...
            # 構建檔案名稱 - CASE版本
            filename = f"CCD1_CASE_Detection_{timestamp}_F{result.case_f_count}_B{result.case_b_count}_S{result.stack_count}_M{result.model_id_used}.jpg"
            ...
            if success:
                ...
                print(f"   檢測結果: CASE_F={result.case_f_count}, CASE_B={result.case_b_count}, STACK={result.stack_count}, 模型={result.model_id_used}")
            else:
    ...
    def _should_save_image(self, result: YOLODetectionResult) -> bool:
        ...
        save_mode = getattr(self, 'image_save_mode', ImageSaveMode.SUCCESS_ONLY)
    ...
    def _create_yolo_visualization(self, image: np.ndarray, result: YOLODetectionResult):
        """創建YOLOv11檢測結果可視化 - 記憶體優化版，CASE版本"""
        try:
            # 使用原地操作減少記憶體複製
            ...
            for i, (x, y) in enumerate(result.case_b_coords):
                cv2.circle(vis_image, (int(x), int(y)), 12, colors['CASE_B'], -1)
                cv2.circle(vis_image, (int(x), int(y)), 17, (255, 255, 255), 2)
                
                label = f"CASE_B {i+1}"
                cv2.putText(vis_image, label, (int(x-35), int(y-25)), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, colors['CASE_B'], 2)
            ...
            # 添加檢測統計信息
            stats_text = f"YOLOv11_CASE (Model{result.model_id_used}): F={result.case_f_count}, B={result.case_b_count}, S={result.stack_count}"
...
def main():
    """主函數 - CASE增強版 (移除Web介面)"""
    print("=" * 80)
    print("🚀 CCD1視覺控制系統啟動中 (YOLOv11 CASE增強版 - 移除Web介面)...")
    print("📊 功能特性:")
    print("   • YOLOv11物件檢測 (CASE_B/CASE_F/STACK)")
    print("   • 握手式狀態機控制")
    ...
    print("   • 圖片保存控制")
    print("   • 100ms高頻輪詢")
    print("   • 世界座標轉換 (僅CASE_F)")
    print("   • 移除Web介面，純Modbus架構")
    print("=" * 80)
    ...
    try:
        ...
        print("📋 Modbus寄存器映射 (CASE增強版):")
        print("   • 控制: 200-206 (指令/狀態/模型選擇)")
        print("   • 參數: 210-211 (置信度閾值)")
        print("   • 結果: 240-259 (CASE_F/CASE_B/STACK檢測結果)")
        print("   • 世界座標: 260-279 (CASE_F座標轉換)")
        print("   • 統計: 280-299 (時間/計數/版本)")
        print("   • 系統: 295(記憶體MB), 296(重載觸發), 297(重載狀態)")
        print("   • 圖片: 298(保存控制), 299(保存模式)")
        
        print("🔗 控制方式:")
        print("   1. 透過PLC寫入寄存器200(控制指令)")
        print("   2. 透過寄存器202選擇YOLO模型(0-20)")
        print("   3. 透過寄存器296觸發系統重載")
        print("   4. 透過寄存器298/299控制圖片保存")
        print("   5. 監控寄存器295獲取記憶體使用量")
        print("   6. 檢測結果直接寫入Modbus寄存器")
        print("=" * 80)
