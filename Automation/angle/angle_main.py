...
class AngleAdjustmentService:
    ...
    def connect_motor(self, port: str = "COM5", baudrate: int = 19200) -> bool:
        """連接馬達驅動器"""
        try:
    ...
    def read_ccd3_angle(self) -> Optional[float]:
        """讀取CCD3檢測到的角度 - 修正地址偏移問題"""
        try:
            if not self.modbus_client or not self.modbus_client.connected:
                print("Modbus未連接，無法讀取CCD3角度")
                return None
            
            print(f"開始讀取CCD3檢測結果 (基地址: {self.ccd3_base_address})")
            
            # 正確：讀取CCD3檢測結果寄存器 840-844
            # 840 = 800 + 40, 841 = 800 + 41, 842 = 800 + 42, 843 = 800 + 43, 844 = 800 + 44
            result = self.modbus_client.read_holding_registers(
                address=840, count=5, slave=1  # 直接使用絕對地址840-844
            )
            
            if result.isError():
                print(f"讀取CCD3角度失敗: {result}")
                return None
            ...
            print(f"CCD3寄存器原始數據:")
            print(f"  840 (成功標誌): {success_flag}")
            print(f"  841 (中心X): {center_x}")
            print(f"  842 (中心Y): {center_y}")  
            print(f"  843 (角度高位): {angle_high}")
            print(f"  844 (角度低位): {angle_low}")
            
            if success_flag != 1:
                print("CCD3檢測未成功，無有效角度數據")
                print(f"成功標誌值: {success_flag} (期望值: 1)")
                return None
            ...
            print(f"CCD3角度計算結果:")
            print(f"  32位整數值: {angle_int}")
            print(f"  最終角度: {angle_degrees:.2f}度")
    ...
    def trigger_ccd3_detection(self) -> bool:
        """觸發CCD3角度檢測 - 修正指令發送和狀態檢查"""
        try:
            if not self.modbus_client or not self.modbus_client.connected:
                print("Modbus未連接，無法觸發CCD3檢測")
                return False
            
            print("=== 觸發CCD3角度檢測流程開始 ===")
            
            # 步驟1：檢查CCD3當前狀態
            print("步驟1：檢查CCD3當前狀態...")
            status_result = self.modbus_client.read_holding_registers(
                address=801, count=1, slave=1  # 直接使用絕對地址801
            )
            
            if not status_result.isError():
                current_status = status_result.registers[0]
                ready = bool(current_status & (1 << 0))    # bit 0: Ready
                running = bool(current_status & (1 << 1))  # bit 1: Running
                alarm = bool(current_status & (1 << 2))    # bit 2: Alarm
                initialized = bool(current_status & (1 << 3))  # bit 3: Initialized
                
                print(f"CCD3當前狀態 (寄存器801): {current_status}")
                print(f"  Ready: {ready}")
                ...
                if alarm:
                    print("警告: CCD3處於Alarm狀態，無法執行檢測")
                    return False
                    
                if not initialized:
                    print("警告: CCD3未初始化，無法執行檢測")
                    return False
                    
                if running:
                    print("警告: CCD3正在執行中，等待完成...")
                    # 可以選擇等待或直接返回
            else:
                print("警告: 無法讀取CCD3狀態，繼續嘗試發送指令")
            
            # 步驟2：清零CCD3指令寄存器
            print("步驟2：清零CCD3指令寄存器...")
            clear_result = self.modbus_client.write_register(
                address=800, value=0, slave=1  # 直接使用絕對地址800
            )
            if clear_result.isError():
                print(f"清零CCD3指令失敗: {clear_result}")
                return False
            ...
            # 步驟3：發送拍照+角度檢測指令
            print("步驟3：發送CCD3拍照+角度檢測指令 (指令16)...")
            command_result = self.modbus_client.write_register(
                address=800, value=16, slave=1  # 直接使用絕對地址800
            )
            
            if command_result.isError():
                print(f"發送CCD3檢測指令失敗: {command_result}")
                return False
            
            print("✓ CCD3檢測指令發送成功")
            
            # 步驟4：驗證指令是否被接受
            time.sleep(0.2)  # 等待CCD3處理指令
            verify_result = self.modbus_client.read_holding_registers(
                address=800, count=2, slave=1  # 讀取指令和狀態
            )
            
            if not verify_result.isError():
                command_reg = verify_result.registers[0]  # 800: 指令寄存器
                status_reg = verify_result.registers[1]   # 801: 狀態寄存器
                
                running_after = bool(status_reg & (1 << 1))  # bit 1: Running
                
                print(f"指令發送後狀態驗證:")
                print(f"  指令寄存器800: {command_reg}")
                print(f"  狀態寄存器801: {status_reg}")
                print(f"  Running狀態: {running_after}")
                
                if running_after:
                    print("✓ CCD3已開始執行檢測")
                    return True
                else:
                    print("⚠️ CCD3未進入Running狀態，但指令已發送")
                    return True  # 仍然返回True，因為指令已發送
            else:
                print("⚠️ 無法驗證指令狀態，但指令已發送")
                return True
    ...
    def wait_ccd3_complete(self, timeout: float = 15.0) -> bool:
        """等待CCD3檢測完成 - 增加超時時間和更詳細的狀態監控"""
        try:
            print("=== 等待CCD3檢測完成 ===")
            start_time = time.time()
            check_count = 0
            ...
            while time.time() - start_time < timeout:
                ...
                if result.isError():
                    print(f"第{check_count}次狀態檢查失敗: {result}")
                    time.sleep(0.5)
                    continue
                ...
                alarm = bool(status_register & (1 << 2))    # bit 2: Alarm
                initialized = bool(status_register & (1 << 3))  # bit 3: Initialized
                
                print(f"第{check_count}次狀態檢查 ({elapsed:.1f}s):")
                print(f"  狀態寄存器801: {status_register}")
                print(f"  Ready: {ready}, Running: {running}, Alarm: {alarm}, Init: {initialized}")
                
                # 檢查是否出現異常
                if alarm:
                    print("❌ CCD3檢測過程中出現Alarm，檢測失敗")
                    return False
                
                # 檢查是否完成：Ready=1且Running=0
                if ready and not running:
                    print("✓ CCD3檢測完成 (Ready=1, Running=0)")
                    
                    # 額外檢查：讀取檢測結果確認是否真的完成
                    result_check = self.modbus_client.read_holding_registers(
                        address=840, count=1, slave=1  # 檢查成功標誌
                    )
                    
                    if not result_check.isError():
                        success_flag = result_check.registers[0]
                        print(f"檢測結果確認 (寄存器840): 成功標誌={success_flag}")
                        
                        # 清零指令寄存器，為下次檢測做準備
                        print("清零CCD3指令寄存器...")
                        clear_result = self.modbus_client.write_register(
                            address=800, value=0, slave=1
                        )
                        if clear_result.isError():
                            print(f"清零指令失敗: {clear_result}")
                        else:
                            print("✓ CCD3指令已清零，系統Ready可接受新指令")
                        
                        return True
                    else:
                        print("⚠️ 無法讀取檢測結果，但狀態顯示完成")
                        return True
                
                # 如果長時間Running，打印進度
                if running and check_count % 10 == 0:
                    print(f"⏳ CCD3仍在檢測中... (已等待{elapsed:.1f}秒)")
            ...
            # 超時情況下也嘗試清零指令
            try:
                print("超時情況下嘗試清零CCD3指令...")
                self.modbus_client.write_register(address=800, value=0, slave=1)
...
def auto_initialize_system():
    ...
    print("步驟1: 自動連接馬達驅動器...")
    motor_success = angle_service.connect_motor("COM5", 19200)
    if motor_success:
        print("✓ 馬達驅動器連接成功")
    else:
        print("✗ 馬達驅動器連接失敗")
...
if __name__ == '__main__':
    ...
    print(f"馬達驅動器: COM5, 19200, Slave {angle_service.motor_slave_id}")
