...
class AngleAppService:
    ...
    def load_config(self):
        """載入配置檔案 - CASE模式"""
        default_config = {
            "module_id": "Angle_Adjustment_Web_App_CASE",
            "description": "CASE模式角度調整Web應用",
            "modbus_tcp": {
                "host": "127.0.0.1",
                "port": 502,
                "unit_id": 1,
        ...
        try:
            ...
            self.motor_baudrate = config['motor_config']['baudrate']
            self.case_mode = config['motor_config']['case_mode']
            
        except Exception as e:
            print(f"配置檔案載入錯誤: {e}")
            self.refresh_interval = 1.0
            self.motor_baudrate = 19200
            self.case_mode = True
    
    def connect_modbus(self) -> bool:
        """連接Modbus TCP服務器"""
        try:
            print(f"CASE模式正在連接Modbus TCP服務器: {self.server_ip}:{self.server_port}")
            ...
            if self.modbus_client.connect():
                print(f"CASE模式角度調整Web應用已連接到Modbus服務器")
    ...
    def read_status_registers(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'motor_baudrate': self.motor_baudrate
        }
    ...
    def read_result_registers(self) -> Dict[str, Any]:
        ...
        result = {
            ...
            'error_count': 0,
            'runtime': 0,
            'case_mode_formula': "9000 - (CCD3角度 × 100)"
        }
...
def connect_modbus():
    ...
    if success:
        return jsonify({'success': True, 'message': f'CASE模式已連接到Modbus服務器 {ip}:{port}'})
    else:
...
def angle_correction():
    ...
    if success:
        ...
        return jsonify({'success': True, 'message': 'CASE模式角度校正指令已發送'})
    else:
...
def motor_reset():
    ...
    if success:
        ...
        return jsonify({'success': True, 'message': 'CASE模式馬達重置指令已發送'})
    else:
        return jsonify({'success': False, 'message': '指令發送失敗'})
...
def error_reset():
    ...
    if success:
        ...
        return jsonify({'success': True, 'message': 'CASE模式錯誤重置指令已發送'})
    else:
        return jsonify({'success': False, 'message': '指令發送失敗'})
...
def handle_connect():
    print("Web客戶端已連接")
    emit('message', {'data': 'CASE模式角度調整系統Web介面已連接'})
...
def handle_send_command(data):
    ...
    emit('command_response', {
        'success': success,
        'command': command,
        'message': f'CASE模式指令 {command} {"已發送" if success else "發送失敗"}'
    })

if __name__ == '__main__':
    print("CASE模式角度調整系統Web應用啟動中...")
    ...
    print(f"馬達配置: COM5, {angle_app_service.motor_baudrate}波特率")
    print(f"運行模式: CASE模式")
    print(f"計算公式: 馬達位置 = 9000 - (CCD3角度 × 100)")
    
    try:
        socketio.run(app, host='localhost', port=5087, debug=False)
