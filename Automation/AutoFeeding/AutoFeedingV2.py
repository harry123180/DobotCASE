...
# -*- coding: utf-8 -*-
"""
AutoFeeding_main.py - CASE專案獨立入料檢測模組 (超強化版)
基地址：900-999
新增功能：
1. 監控200地址是否卡16，自動歸零
2. 監控CCD1報警狀態，自動初始化
...
class AutoFeedingModule:
    """CASE AutoFeeding獨立模組 (超強化版)"""
    
    def __init__(self, modbus_host: str = "127.0.0.1", modbus_port: int = 502):
        self.logger = setup_logging('AutoFeeding_CASE')
        ...
        self.case_f_coords = (0.0, 0.0)
        self.case_f_taken = False
        ...
        self.logger.info(f"CASE AutoFeeding獨立模組初始化 (超強化版) - 基地址{self.BASE_ADDRESS}")
        ...
        self.logger.info(f"  ✓ CASE_F檢測與CASE_B背景監控")
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'autofeeding_case_config.json')
    ...
    def init_registers(self):
        ...
        try:
            ...
            self.logger.info("寄存器初始化完成 (CASE版本-新增CCD1監控和記憶體管理)")
        except Exception as e:
    ...
    def update_status_registers(self):
        ...
        try:
            ...
            # 更新CASE_F狀態
            self.write_register(940, 1 if self.case_f_available else 0)
            print("1 if self.case_f_available else 0",1 if self.case_f_available else 0)
            if self.case_f_available:
                self.write_32bit_register(941, 942, self.case_f_coords[0])
                self.write_32bit_register(943, 944, self.case_f_coords[1])
        except Exception as e:
    ...
    def check_flow_and_progress_status(self) -> bool:
        try:
            ...
            # 根據暫停判斷更新狀態
            if should_pause:
                if self.status not in [AutoFeedingStatus.FLOW1_PAUSED, AutoFeedingStatus.FLOW1_PAUSED]:
                ...
                # 確保在不暫停時設置為RUNNING狀態
                if self.status in [AutoFeedingStatus.FLOW1_PAUSED, AutoFeedingStatus.FLOW1_PAUSED]:
    ...
    def check_coords_taken(self):
        """檢查座標是否被Flow1讀取"""
        if not self.case_f_available:
            return
        
        try:
            coords_taken = self.read_register(945)
            if coords_taken == 1:
                self.logger.info("座標已被Flow1讀取，清除CASE_F狀態")
                self.case_f_available = False
                self.case_f_coords = (0.0, 0.0)
                self.logger.info(f"CASE_F座標為:{self.case_f_coords}")
                self.case_f_taken = True
                ...
                self.logger.info("CASE_F狀態已清除，繼續檢測新的正面物件")
        except Exception as e:
    ...
    def check_for_anomaly(self, current_result: CCD1DetectionResult) -> bool:
        ...
        prev_had_large = (self.anomaly_detection.previous_case_f_count >= large_threshold or 
                         self.anomaly_detection.previous_total_count >= large_threshold)
        
        current_has_drop = (current_result.case_f_count <= drop_threshold and 
                           current_result.total_detections <= drop_threshold)
        
        if prev_had_large and current_has_drop:
            self.logger.warning(f"檢測到異常：前次大量檢測(CASE_F={self.anomaly_detection.previous_case_f_count}, 總數={self.anomaly_detection.previous_total_count})，當前驟減(CASE_F={current_result.case_f_count}, 總數={current_result.total_detections})")
            return True
    ...
    def update_anomaly_detection(self, result: CCD1DetectionResult):
        """更新異常檢測記錄"""
        self.anomaly_detection.previous_case_f_count = result.case_f_count
    ...
    def trigger_ccd1_detection(self) -> CCD1DetectionResult:
        ...
        case_b_count = self.read_register(241) or 0
    ...
    def trigger_vp_vibration(self) -> bool:
        ...
        # success &= self.write_register(322, 60)
        # success &= self.write_register(323, 10)
        # success &= self.write_register(324, int(time.time()) % 65535)
    ...
    def set_case_f_available(self, coords: Tuple[float, float]):
        ...
        self.logger.info(f"CASE_F已就緒: {coords}, Flow1可直接讀取座標")
    def should_recheck_detection(self, current_case_f: int, current_background: int) -> bool:
        """
        判斷是否需要重新檢測
        
        條件：
        1. 上次CASE_F>=3，這次CASE_F=0
        2. 上次背景物件>=10，這次背景物件>=2
        
        Args:
            current_case_f: 當前檢測到的CASE_F數量
        ...
        """
        # 第一次檢測，沒有歷史數據
        if not hasattr(self, 'last_case_f_count') or not hasattr(self, 'last_background_count'):
            return False
        
        # 檢查CASE_F異常歸零
        case_f_abnormal = (self.last_case_f_count >= 3 and current_case_f < 1 )
        
        # 檢查背景物件仍然存在
        background_sufficient = (self.last_background_count >= 3 and current_background < 2)
        
        # 兩個條件都滿足才重檢
        return case_f_abnormal and background_sufficient
    def feeding_cycle(self) -> bool:
        ...
        try:
            ...
            current_motion_flow = self.read_register(self.CURRENT_MOTION_FLOW)
            motion_progress = self.read_register(self.MOTION_PROGRESS)
            if not(current_motion_flow ==1 and motion_progress<50):
                ...
                need_recheck = self.should_recheck_detection(
                    detection_result.case_f_count, 
                    background_count
                )
                self.logger.warning(f"膩的確可{need_recheck}")
                if need_recheck:
                    self.logger.warning(f"檢測結果異常，1秒後重新檢測")
                    self.logger.warning(f"  上次CASE_F={self.last_case_f_count}, 這次CASE_F={detection_result.case_f_count}")
                    ...
                    if not(current_motion_flow ==1 and motion_progress<50):
                        ...
                        if retry_result.operation_success:
                            ...
                            self.logger.info(f"重檢結果: CASE_F={detection_result.case_f_count}, 背景物件={background_count}")
                        else:
                            self.logger.error("重檢失敗，使用原始檢測結果")
                
                # 記錄當前檢測結果供下次比較
                self.last_case_f_count = detection_result.case_f_count
                self.last_background_count = background_count
                
                self.logger.info(f"週期{self.cycle_count} 檢測結果:")
                self.logger.info(f"  CASE_F={detection_result.case_f_count}")
                self.logger.info(f"  CASE_B={case_b_count}")
                ...
                if target_coords:
                    ...
                    self.flow4_consecutive_count = 0
                    self.logger.info(f"找到保護區內CASE_F: {target_coords}")
                    
                    # 設置CASE_F可用狀態
                    self.set_case_f_available(target_coords)
                    
                    # 檢查是否需要供料
                    if background_count < 4 and self.case_f_available ==False:
                        self.logger.info(f"背景物件不足({background_count}<4)，但有CASE_F可夾取，暫不供料")
                        self.logger.info(f"等待Flow1夾取後再評估是否需要補料")
                    else:
                        self.logger.info(f"背景物件充足({background_count}>=4)，CASE_F已就緒")
                else:
                    # 保護區無CASE_F - 根據背景物件數量決定動作
                    if background_count < 4 and self.case_f_available ==False :
                        # 背景物件不足且無正面，觸發直振供料
                        self.logger.info(f"背景物件不足({background_count}<4)且無CASE_F，觸發Flow4直振供料")
                        #time.sleep(2)
                        retry_result = self.trigger_ccd1_detection()
                        if retry_result.operation_success:
                            detection_result = retry_result
                            ...
                            #Flow4直振供料完成
                                ...
                                time.sleep(2)
                                self.logger.info(f"直振打完等一秒馬上VP")
                    ...
                    elif(self.case_f_available ==False ):
                        # 背景物件充足但無正面，VP震動重檢
                        self.logger.info(f"背景物件充足({background_count}>=4)但無CASE_F，VP震動散開重檢")
                        self.flow4_consecutive_count = 0
    ...
    def main_loop(self):
        ...
        while True:
            try:
                ...
                if loop_count % 200 == 1:
                    self.logger.debug(f"主循環 {loop_count}: running={self.running}, status={self.status.name}, flow1_active={self.flow1_active}, progress_blocking={self.progress_blocking}, case_f_available={self.case_f_available}")
