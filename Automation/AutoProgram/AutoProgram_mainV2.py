...
# -*- coding: utf-8 -*-
"""
AutoProgram_main.py - CASE專案機械臂協調控制模組 (修正prepare_done邏輯版)
基地址：1300-1399
修正策略：嚴格控制prepare_done狀態機，防止Flow1重複觸發
Flow5 done->AutoFeeding done ->AutoProgram get coordinates-> Flow1->Flow1 get AutoProgram coordinates->work flow
...
class AutoProgramController:
    ...
    def __init__(self, modbus_host: str = "127.0.0.1", modbus_port: int = 502):
        ...
        # 基地址配置
        self.BASE_ADCASEESS = 1300
        
        # AutoFeeding模組地址 (CASE專案使用940-945)
        ...
        self.DOBOT_CURRENT_MOTION_FLOW = 1201 # 當前運動Flow (0=無, 1=Flow1, 2=Flow5, 5=Flow5)
        ...
        print("CASE專案機械臂協調控制模組初始化完成 (提前座標讀取版)")
        print(f"Modbus服務器: {modbus_host}:{modbus_port}")
        print(f"AutoProgram基地址: {self.BASE_ADCASEESS}")
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "monitoring": {
                "case_f_check_interval": 0.1,
                "flow5_check_interval": 0.2,
            ...
            "project_info": {
                "project_name": "CASE",
                "detection_types": ["CASE_F", "STACK"],
                "flow_config": "Flow1+Flow5",
                "coordinate_strategy": "AutoProgram_PreRead"
            }
        }
        
        try:
            config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'case_autoprogram_config.json')
            if os.path.exists(config_path):
                with open(config_path, 'r', encoding='utf-8') as f:
                    loaded_config = json.load(f)
                    default_config.update(loaded_config)
                print(f"已載入CASE專案配置檔案: {config_path}")
            else:
                with open(config_path, 'w', encoding='utf-8') as f:
                    json.dump(default_config, f, indent=2, ensure_ascii=False)
                print(f"已創建CASE專案預設配置檔案: {config_path}")
    ...
    def init_system_registers(self):
        ...
        try:
            ...
            self.write_register(self.BASE_ADCASEESS, SystemStatus.STOPPED.value)
            ...
            print("CASE專案AutoProgram系統寄存器初始化完成")
            print(f"修正後座標存儲寄存器: {self.AP_COORDS_AVAILABLE}-{self.AP_TARGET_Y_LOW}")
            print(f"與Flow1讀取地址1340-1343完全一致")
        except Exception as e:
            print(f"系統寄存器初始化失敗: {e}")
    
    def read_register(self, adcaseess: int) -> Optional[int]:
        """讀取單個寄存器"""
        try:
            result = self.modbus_client.read_holding_registers(adcaseess, count=1, slave=1)
    ...
    def write_register(self, adcaseess: int, value: int) -> bool:
        """寫入單個寄存器"""
        try:
            result = self.modbus_client.write_register(adcaseess, value, slave=1)
            return not result.isError()
        except Exception:
            return False
    
    def read_32bit_coordinate(self, high_adcase: int, low_adcase: int) -> float:
        """讀取32位世界座標並轉換為實際值"""
        high_val = self.read_register(high_adcase)
        low_val = self.read_register(low_adcase)
    ...
    def write_32bit_coordinate(self, high_adcase: int, low_adcase: int, value: float) -> bool:
        """寫入32位世界座標"""
        try:
            print(f"[AutoProgram] 寫入32位座標: 值={value:.2f}, 高地址={high_adcase}, 低地址={low_adcase}")
            ...
            # 寫入寄存器
            high_success = self.write_register(high_adcase, high_val)
            low_success = self.write_register(low_adcase, low_val)
            
            print(f"[AutoProgram] 寄存器寫入結果: 高地址{high_adcase}={high_success}, 低地址{low_adcase}={low_success}")
    ...
    def get_autofeeding_status(self) -> Dict[str, Any]:
        ...
        if case_f_available == 1:
            ...
            case_f_available = self.write_register(self.AF_CASE_F_AVAILABLE,0) #清除case_f可用標誌
        return {
    ...
    def read_and_validate_autofeeding_coordinates(self) -> Optional[Tuple[float, float]]:
        ...
        print(f"[AutoProgram] AutoFeeding狀態: CASE_F可用={af_status['case_f_available']}, 座標已讀取={af_status['coords_taken']}")
        
        if not af_status['case_f_available']:
            #print(f"[AutoProgram] AutoFeeding CASE_F不可用 (940=0)")
            return None
    ...
    def coordination_cycle(self):
        """機械臂協調控制週期 - 修正版：Flow1完成後持續檢查Flow5"""
        try:
            ...
            if self.coordination_cycle_count % 50 == 0:
                ...
                flow5_complete = self.read_register(self.DOBOT_FLOW5_COMPLETE)  # 直接讀取1206
            ...
            if self.check_flow1_complete() and not self.prepare_done:
                ...
                print("[AutoProgram] 機台準備就緒，等待Flow5完成")
                print("[AutoProgram] Flow1完成狀態(1204=1)保持不變，供自動交握檢查")
                return  # Flow1完成處理後返回，下一週期檢查Flow5
            ...
            if not self.prepare_done:
                ...
                if not self.coords_ready:
                    ...
                    if coords:
                        ...
                        if self.coordination_cycle_count % 25 == 0:
                            af_status = self.get_autofeeding_status()
                            print(f"[AutoProgram] 等待AutoFeeding新座標... (940={af_status.get('case_f_available', 'N/A')})")
                ...
                # === prepare_done=True：等待Flow5完成，完全不檢查AutoFeeding ===
                if self.coordination_cycle_count % 100 == 0:
                    flow5_status = self.read_register(self.DOBOT_FLOW5_COMPLETE)
                    ...
                    print(f"[AutoProgram] 目前Flow5完成狀態(1206): {flow5_status}")
    ...
    def start(self):
        ...
        print("[AutoProgram] === 啟動CASE專案機械臂協調控制系統 (防重複觸發版) ===")
        self.running = True
        ...
        print("[AutoProgram] CASE專案協調控制系統已啟動")
        ...
        print(f"[AutoProgram]   - prepare_done=True時等待Flow5完成")
        print(f"[AutoProgram]   - Flow5完成後prepare_done重置為False")
        ...
        print(f"[AutoProgram] 執行邏輯: 座標準備完成 → 檢查Flow1狀態 → 觸發Flow1 → 等待完成 → prepare_done=True → 等待Flow5")
    ...
    def stop(self):
        ...
        print("[AutoProgram] === 停止CASE專案機械臂協調控制系統 ===")
        self.running = False
        self.system_status = SystemStatus.STOPPED
        ...
        print("[AutoProgram] CASE專案協調控制系統已停止")
    ...
    def _coordination_loop(self):
        ...
        print("[AutoProgram] CASE專案協調控制主循環已啟動")
    ...
    def print_statistics(self):
        ...
        print(f"狀態機邏輯: prepare_done=False時觸發Flow1, prepare_done=True時等待Flow5")
...
def main():
    """主程序"""
    print("CASE專案機械臂協調控制模組啟動 (主動觸發Flow1版)")
    print("主動觸發策略: AutoProgram讀取座標後立即主動觸發Flow1")
    print("執行流程: Flow5 done->AutoFeeding done->AutoProgram get coordinates->AutoProgram trigger Flow1->Flow1 get coordinates->work flow")
